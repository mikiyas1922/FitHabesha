...
function FeatureCard({ icon: Icon, title, description, highlight = false, className = '' }) {
  return (
    <div
      className={`rounded-2xl border p-6 flex flex-col backdrop-blur-sm transition-all duration-500 hover:scale-105 hover:-translate-y-1 ${
        highlight
          ? 'bg-linear-to-br from-primary to-primary-dark border-primary text-dark shadow-xl shadow-primary/30'
          : 'bg-linear-to-br from-dark-card to-dark-card/80 border-white/10 text-white hover:border-primary/50 hover:shadow-xl hover:shadow-primary/20'
      } ${className}`}
    >
      <div
        className={`flex size-11 items-center justify-center rounded-xl mb-4 ${
          highlight ? 'bg-dark/20 text-dark shadow-lg' : 'bg-linear-to-br from-primary/20 to-primary/10 text-primary shadow-lg shadow-primary/20'
        }`}
      >
